...
use Google\Web_Stories_Dependencies\AmpProject\AmpWP\Admin\ValidationCounts;
/**
 * Class AMP_Validation_Error_Taxonomy
 *
 * @since 1.0
...
class AMP_Validation_Error_Taxonomy
{
     ...
     * @var int
     */
    const ACKNOWLEDGED_VALIDATION_ERROR_BIT_MASK = 2;
    // === 0b10.
    /**
     ...
     * @var int
     */
    const ACCEPTED_VALIDATION_ERROR_BIT_MASK = 1;
    // === 0b01.
    /**
    ...
    // == 0b00 == ^ACKNOWLEDGED_VALIDATION_ERROR_BIT_MASK | ^ACCEPTED_VALIDATION_ERROR_BIT_MASK.
    /**
    ...
    // == 0b01 == ^ACKNOWLEDGED_VALIDATION_ERROR_BIT_MASK | ACCEPTED_VALIDATION_ERROR_BIT_MASK.
    /**
     * Term group for validation_error terms that the accepts and thus can be sanitized and does not disable AMP.
     *
     * @var int
     */
    const VALIDATION_ERROR_ACK_ACCEPTED_STATUS = 3;
    // == 0b11 == ACKNOWLEDGED_VALIDATION_ERROR_BIT_MASK | ACCEPTED_VALIDATION_ERROR_BIT_MASK.
    /**
     * Term group for validation_error terms that the user flags as being blockers to enabling AMP.
     *
     * @var int
     */
    const VALIDATION_ERROR_ACK_REJECTED_STATUS = 2;
    // == 0b10 == ACKNOWLEDGED_VALIDATION_ERROR_BIT_MASK | ^ACCEPTED_VALIDATION_ERROR_BIT_MASK.
    /**
    ...
    public static function register()
    {
        ...
        $show_in_menu = $dev_tools_user_access->is_user_enabled() || isset($_GET['post_type']) && \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG === $_GET['post_type'] || isset($_GET['post'], $_GET['action']) && 'edit' === $_GET['action'] && \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG === \get_post_type((int) $_GET['post']) || isset($_GET['taxonomy']) && self::TAXONOMY_SLUG === $_GET['taxonomy'] || isset($_GET[self::TAXONOMY_SLUG]);
        \register_taxonomy(self::TAXONOMY_SLUG, \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, [
            'labels' => [
                ...
                'add_new_item' => \__('Add New Validation Error', 'amp'),
                // Makes no sense.
                'new_item_name' => \__('New Validation Error Hash', 'amp'),
                // Makes no sense.
            ...
            'public' => \false,
            'show_ui' => \true,
            // @todo False because we need a custom UI.
            'show_tagcloud' => \false,
            'show_in_quick_edit' => \false,
            'hierarchical' => \false,
            // Or true? Code could be the parent term?
            'show_in_menu' => $show_in_menu,
            'meta_box_cb' => \false,
            'capabilities' => [
                'manage_terms' => \Google\Web_Stories_Dependencies\AMP_Validation_Manager::VALIDATE_CAPABILITY,
                // Needed to give access to the term list table.
                'delete_terms' => \Google\Web_Stories_Dependencies\AMP_Validation_Manager::VALIDATE_CAPABILITY,
                // Needed so the checkbox (cb) table column will work.
                'assign_terms' => 'do_not_allow',
                // Block assign_terms since associating terms with posts is done programmatically.
    ...
    public static function delete_empty_terms()
    {
        ...
        foreach ($empty_term_ids as $term_id) {
            if (\true === self::delete_empty_term($term_id)) {
                $deleted_count++;
            }
        }
    ...
    public static function delete_empty_term($term_id)
    {
        ...
        if (\true === \wp_delete_term($term->term_id, self::TAXONOMY_SLUG)) {
            return \true;
        }
        return \false;
    }
    ...
    public static function sanitize_term_status($status, $options = [])
    {
        ...
        $statuses = \array_intersect([self::VALIDATION_ERROR_NEW_REJECTED_STATUS, self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS, self::VALIDATION_ERROR_ACK_REJECTED_STATUS], $statuses);
    ...
    {
        global $wpdb;
        return $wpdb->prepare('IN ( ' . \implode(', ', \array_fill(0, \count($groups), '%d')) . ' )', $groups);
    }
    /**
    ...
    public static function prepare_validation_error_taxonomy_term($error)
    {
        ...
        return ['slug' => $term_slug, 'name' => $term_slug, 'description' => $description];
    }
    /**
    ...
    public static function is_validation_error_sanitized($error)
    {
        ...
        return self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS === $sanitization['status'] || self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS === $sanitization['status'];
    }
    /**
    ...
    public static function get_validation_error_sanitization($error)
    {
        ...
        $statuses = [self::VALIDATION_ERROR_NEW_REJECTED_STATUS, self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS, self::VALIDATION_ERROR_ACK_REJECTED_STATUS];
        if (!empty($term) && \in_array($term->term_group, $statuses, \true)) {
            $term_status = $term->term_group;
        } else {
            $term_status = \Google\Web_Stories_Dependencies\AMP_Validation_Manager::is_sanitization_auto_accepted($error) ? self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS : self::VALIDATION_ERROR_NEW_REJECTED_STATUS;
        }
        $forced = \false;
        $status = $term_status;
        ...
        if (isset(\Google\Web_Stories_Dependencies\AMP_Validation_Manager::$validation_error_status_overrides[$term_data['slug']])) {
            $status = \Google\Web_Stories_Dependencies\AMP_Validation_Manager::$validation_error_status_overrides[$term_data['slug']];
            $forced = 'with_preview';
        }
        /**
    ...
    public static function accept_validation_errors($acceptable_errors)
    {
        ...
        \add_filter('amp_validation_error_sanitized', static function ($sanitized, $error) use($acceptable_errors) {
            if (\true === $acceptable_errors) {
                return \true;
            }
            if (isset($acceptable_errors[$error['code']])) {
                if (\true === $acceptable_errors[$error['code']]) {
                    return \true;
                }
                foreach ($acceptable_errors[$error['code']] as $acceptable_error_props) {
                    if (\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::is_array_subset($error, $acceptable_error_props)) {
                        return \true;
                    }
                }
            }
    ...
    {
        $args = \array_merge(['group' => null], $args);
        ...
        $groups = null;
        if (isset($args['group'])) {
            $groups = self::sanitize_term_status($args['group'], ['multiple' => \true]);
        }
        $filter = static function ($clauses) use($groups) {
            $clauses['where'] .= ' AND t.term_group ' . \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::prepare_term_group_in_sql($groups);
            return $clauses;
        };
        if (isset($args['group'])) {
    ...
    public static function filter_posts_where_for_validation_error_status($where, \WP_Query $query)
    {
        ...
        if (!\in_array(\Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, (array) $query->get('post_type'), \true)) {
            return $where;
        }
        $error_statuses = [];
        if (\false !== $query->get(self::VALIDATION_ERROR_STATUS_QUERY_VAR, \false)) {
            $error_statuses = self::sanitize_term_status($query->get(self::VALIDATION_ERROR_STATUS_QUERY_VAR), ['multiple' => \true]);
        }
        ...
        $is_error_type_present = \in_array($error_type, self::get_error_types(), \true);
        ...
        if ($is_error_type_present) {
            $sql_select .= $wpdb->prepare(" AND {$wpdb->term_taxonomy}.description LIKE %s ", '%"type":"' . $wpdb->esc_like($error_type) . '"%');
        }
        $sql_select .= ' LIMIT 1 ';
    ...
    public static function summarize_validation_errors($validation_errors)
    {
        ...
        foreach ($validation_errors as $validation_error) {
            ...
            if (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_TAG === $code) {
            ...
            } elseif (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_ATTR === $code) {
            ...
            } elseif (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_PROCESSING_INSTRUCTION === $code) {
        ...
        $results = \array_merge([self::SOURCES_INVALID_OUTPUT => self::summarize_sources($sources)], \compact('removed_elements', 'removed_attributes', 'removed_pis'), $results);
        return $results;
    }
    /**
     * Summarize sources.
    ...
    public static function add_admin_hooks()
    {
        ...
        \add_filter('manage_' . \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG . '_sortable_columns', [__CLASS__, 'add_single_post_sortable_columns']);
        ...
        \add_filter('removable_query_args', static function ($query_vars) {
            $query_vars[] = 'amp_actioned';
            $query_vars[] = 'amp_actioned_count';
            $query_vars[] = 'amp_validation_errors_not_deleted';
            $query_vars[] = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::VALIDATION_ERRORS_CLEARED_QUERY_VAR;
            return $query_vars;
        });
        ...
        \add_filter('query_vars', static function ($query_vars) {
            $query_vars[] = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::VALIDATION_ERROR_STATUS_QUERY_VAR;
            $query_vars[] = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::VALIDATION_ERROR_TYPE_QUERY_VAR;
            return $query_vars;
        });
        // Default ordering terms by ID descending so that new terms appear at the top.
        \add_filter('get_terms_defaults', static function ($args, $taxonomies) {
            if ([\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG] === $taxonomies) {
        ...
        \add_action('admin_enqueue_scripts', static function () {
            ...
            if (\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG === $current_screen->taxonomy) {
				...
				');
                \wp_register_style('amp-validation-tooltips', \amp_get_asset_url('css/amp-validation-tooltips.css'), ['wp-pointer'], \AMP__VERSION);
                ...
                \wp_register_script('amp-validation-tooltips', \amp_get_asset_url('js/amp-validation-tooltips.js'), $dependencies, $version, \true);
                \wp_enqueue_style('amp-validation-error-taxonomy', \amp_get_asset_url('css/amp-validation-error-taxonomy.css'), ['common', 'amp-validation-tooltips', 'amp-icons'], \AMP__VERSION);
                \wp_styles()->add_data('amp-validation-error-taxonomy', 'rtl', 'replace');
                \wp_enqueue_script('amp-validation-detail-toggle', \amp_get_asset_url('js/amp-validation-detail-toggle.js'), ['wp-dom-ready', 'wp-i18n', 'amp-validation-tooltips'], \AMP__VERSION, \true);
            }
            if ('post' === $current_screen->base && \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG === $current_screen->post_type) {
                \wp_enqueue_style('amp-validation-single-error-url', \amp_get_asset_url('css/amp-validation-single-error-url.css'), ['common', 'amp-icons'], \AMP__VERSION);
                ...
                $dependencies = $asset['dependencies'];
                $version = $asset['version'];
                \wp_enqueue_script('amp-validation-single-error-url-details', \amp_get_asset_url('js/amp-validation-single-error-url-details.js'), $dependencies, $version, \true);
            }
        });
        ...
        if (\current_user_can('manage_options')) {
            \add_filter('parent_file', static function ($parent_file) {
                if (\get_current_screen()->taxonomy === \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG) {
                    $parent_file = \Google\Web_Stories_Dependencies\AMP_Options_Manager::OPTION_NAME;
                }
                return $parent_file;
            }, 10, 2);
        }
        // Replace the primary column to be error instead of the removed name column..
        \add_filter('list_table_primary_column', static function ($primary_column) {
            if (\get_current_screen() && \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG === \get_current_screen()->taxonomy) {
    ...
    {
        if (self::TAXONOMY_SLUG !== $tax->name || !isset($_POST['post_type']) || \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG !== $_POST['post_type']) {
            return $url;
        }
        // If the error type query var is valid, pass it along in the redirect $url.
        if (isset($_POST[self::VALIDATION_ERROR_TYPE_QUERY_VAR]) && \in_array(
            $_POST[self::VALIDATION_ERROR_TYPE_QUERY_VAR],
            ...
            $url = \add_query_arg(
                ...
                \sanitize_key(\wp_unslash($_POST[self::VALIDATION_ERROR_TYPE_QUERY_VAR])),
                // phpcs:ignore WordPress.Security.NonceVerification.Missing
                $url
            );
        }
        ...
        if (isset($_POST[self::VALIDATION_ERROR_STATUS_QUERY_VAR])) {
            ...
            $groups = self::sanitize_term_status(\wp_unslash($_POST[self::VALIDATION_ERROR_STATUS_QUERY_VAR]), ['multiple' => \true]);
        }
        if (!empty($groups)) {
    ...
    {
        if (self::TAXONOMY_SLUG !== \get_current_screen()->taxonomy || !isset($_GET[self::VALIDATION_ERROR_STATUS_QUERY_VAR])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        self::$should_filter_terms_clauses_for_error_validation_status = \true;
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $groups = self::sanitize_term_status(\wp_unslash($_GET[self::VALIDATION_ERROR_STATUS_QUERY_VAR]), ['multiple' => \true]);
        if (empty($groups)) {
            return;
        }
        \add_filter('terms_clauses', static function ($clauses, $taxonomies) use($groups) {
            if (\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG === $taxonomies[0] && \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::$should_filter_terms_clauses_for_error_validation_status) {
                $clauses['where'] .= ' AND t.term_group ' . \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::prepare_term_group_in_sql($groups);
            }
            return $clauses;
        }, 10, 2);
    ...
    {
        if (self::TAXONOMY_SLUG !== \get_current_screen()->taxonomy || !isset($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        $type = \sanitize_key(\wp_unslash($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR]));
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        if (!\in_array($type, self::get_error_types(), \true)) {
            return;
        }
        \add_filter('terms_clauses', static function ($clauses, $taxonomies) use($type) {
            global $wpdb;
            if (\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG === $taxonomies[0]) {
    ...
    public static function add_order_clauses_from_description_json()
    {
        ...
        $sortable_column_vars = [self::VALIDATION_ERROR_TYPE_QUERY_VAR, self::VALIDATION_DETAILS_ERROR_CODE_QUERY_VAR];
        if (!isset($_GET['orderby']) || !\in_array($_GET['orderby'], $sortable_column_vars, \true)) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        \add_filter('terms_clauses', static function ($clauses) {
            ...
            switch ($_GET['orderby']) {
                // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                case \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::VALIDATION_ERROR_TYPE_QUERY_VAR:
                    $clauses['orderby'] = $wpdb->prepare('ORDER BY SUBSTR(tt.description, LOCATE(%s, tt.description, LOCATE(%s, tt.description)))', '"type":"', '}');
                    break;
                case \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::VALIDATION_DETAILS_ERROR_CODE_QUERY_VAR:
                    $clauses['orderby'] = $wpdb->prepare('ORDER BY SUBSTR(tt.description, LOCATE(%s, tt.description))', '"code":"');
                    break;
            }
            return $clauses;
        }, 10, 2);
    ...
    public static function render_link_to_invalid_urls_screen($taxonomy_name)
    {
        ...
        $post_type_object = \get_post_type_object(\Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG);
        if (!\current_user_can($post_type_object->cap->edit_posts)) {
            return;
        }
        $id = 'link-errors-url';
        \printf('<a href="%s" class="page-title-action" id="%s" hidden style="margin-left: 1rem;">%s</a>', \esc_url(\add_query_arg('post_type', \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, \admin_url('edit.php'))), \esc_attr($id), \esc_html__('View Validated URLs', 'amp'));
        ?>
		<script>
			jQuery( function( $ ) {
        ...
        ?> ).removeAttr( 'hidden' ).insertAfter( $( '.wp-heading-inline' ) );
			} );
		</script>
		<?php 
    }
    ...
    public static function render_error_status_filter()
    {
        ...
        if ('edit-tags' === $screen_base) {
            ...
            $args = ['post_type' => \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, 'update_post_meta_cache' => \false, 'update_post_term_cache' => \false];
            $error_type = \sanitize_key($wp_query->get(self::VALIDATION_ERROR_TYPE_QUERY_VAR));
            if ($error_type && \in_array($error_type, self::get_error_types(), \true)) {
                $args[self::VALIDATION_ERROR_TYPE_QUERY_VAR] = $error_type;
            }
            $with_new_query = new \WP_Query(\array_merge($args, [self::VALIDATION_ERROR_STATUS_QUERY_VAR => [self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, self::VALIDATION_ERROR_NEW_REJECTED_STATUS]]));
            $unreviewed_count = $with_new_query->found_posts;
            $with_rejected_query = new \WP_Query(\array_merge($args, [self::VALIDATION_ERROR_STATUS_QUERY_VAR => [self::VALIDATION_ERROR_NEW_REJECTED_STATUS, self::VALIDATION_ERROR_ACK_REJECTED_STATUS]]));
            $rejected_term_count = $with_rejected_query->found_posts;
            $with_accepted_query = new \WP_Query(\array_merge($args, [self::VALIDATION_ERROR_STATUS_QUERY_VAR => [self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS]]));
        ...
        ?>
		<label for="<?php 
        echo \esc_attr(self::VALIDATION_ERROR_STATUS_QUERY_VAR);
        ?>" class="screen-reader-text"><?php 
        \esc_html_e('Filter by markup status', 'amp');
        ?></label>
		<select name="<?php 
        echo \esc_attr(self::VALIDATION_ERROR_STATUS_QUERY_VAR);
        ?>" id="<?php 
        ...
        ?>">
			<option value="<?php 
        echo \esc_attr(self::NO_FILTER_VALUE);
        ?>"><?php 
        \esc_html_e('All statuses', 'amp');
        ?></option>
			<?php 
        $options_config = [['text' => 'edit' === $screen_base ? \_x('With unreviewed errors', 'terms', 'amp') : \_x('Unreviewed errors', 'terms', 'amp'), 'value' => self::VALIDATION_ERROR_NEW_REJECTED_STATUS . ',' . self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, 'error_count' => $unreviewed_count], ['text' => 'edit' === $screen_base ? \_x('With removed markup', 'terms', 'amp') : \_x('Removed markup', 'terms', 'amp'), 'value' => self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS . ',' . self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS, 'error_count' => $accepted_term_count], ['text' => 'edit' === $screen_base ? \_x('With kept markup', 'terms', 'amp') : \_x('Kept markup', 'terms', 'amp'), 'value' => self::VALIDATION_ERROR_NEW_REJECTED_STATUS . ',' . self::VALIDATION_ERROR_ACK_REJECTED_STATUS, 'error_count' => $rejected_term_count]];
        foreach ($options_config as $option_config) {
            self::output_error_status_filter_option_markup($option_config['text'], $option_config['value'], $option_config['error_count'], $error_status_filter_value);
        }
        ?>
		</select>
		<?php 
    ...
    {
        \printf('<option value="%s" %s>%s <span class="count">(%s)</span></option>', \esc_attr($option_value), \selected($selected_value, $option_value, \false), \esc_html($option_text), \esc_html(\number_format_i18n($error_count)));
    }
    /**
    ...
    {
        $error_type_filter_value = isset($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR]) ? \sanitize_key(\wp_unslash($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR])) : '';
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        /*
        ...
        ?>
		<label for="<?php 
        echo \esc_attr(self::VALIDATION_ERROR_TYPE_QUERY_VAR);
        ?>" class="screen-reader-text"><?php 
        \esc_html_e('Filter by error type', 'amp');
        ?></label>
		<select name="<?php 
        echo \esc_attr(self::VALIDATION_ERROR_TYPE_QUERY_VAR);
        ?>" id="<?php 
        ...
        ?>">
				<?php 
        \esc_html_e('All types of invalid markup', 'amp');
        ?>
			</option>
			<option value="<?php 
        echo \esc_attr(self::HTML_ELEMENT_ERROR_TYPE);
        ?>" <?php 
        \selected($error_type_filter_value, self::HTML_ELEMENT_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
					<?php 
            \esc_html_e('With invalid HTML elements', 'amp');
            ?>
				<?php 
        } else {
            ?>
					<?php 
            \esc_html_e('Invalid HTML elements', 'amp');
            ?>
				<?php 
        }
        ?>
			...
			<option value="<?php 
        echo \esc_attr(self::HTML_ATTRIBUTE_ERROR_TYPE);
        ?>" <?php 
        \selected($error_type_filter_value, self::HTML_ATTRIBUTE_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
					<?php 
            \esc_html_e('With invalid HTML attributes', 'amp');
            ?>
				<?php 
        } else {
            ?>
					<?php 
            \esc_html_e('Invalid HTML attributes', 'amp');
            ?>
				<?php 
        }
        ?>
			...
			<option value="<?php 
        echo \esc_attr(self::JS_ERROR_TYPE);
        ?>" <?php 
        \selected($error_type_filter_value, self::JS_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
					<?php 
            \esc_html_e('With invalid JS', 'amp');
            ?>
				<?php 
        } else {
            ?>
					<?php 
            \esc_html_e('Invalid JS', 'amp');
            ?>
				<?php 
        }
        ?>
			...
			<option value="<?php 
        echo \esc_attr(self::CSS_ERROR_TYPE);
        ?>" <?php 
        \selected($error_type_filter_value, self::CSS_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
					<?php 
            \esc_html_e('With invalid CSS', 'amp');
            ?>
				<?php 
        } else {
            ?>
					<?php 
            \esc_html_e('Invalid CSS', 'amp');
            ?>
				<?php 
        }
        ?>
    ...
    {
        global $wpdb;
        if (!empty($args['search']) && \in_array(self::TAXONOMY_SLUG, $taxonomies, \true)) {
            $clauses['where'] = \preg_replace('#(?<=\\()(?=\\(t\\.name LIKE \')#', $wpdb->prepare('(tt.description LIKE %s) OR ', '%' . $wpdb->esc_like($args['search']) . '%'), $clauses['where']);
        }
        return $clauses;
    }
    /**
    ...
    {
        if (!(self::TAXONOMY_SLUG === \get_current_screen()->taxonomy || \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG === \get_current_screen()->post_type)) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        ...
        if (!empty($_GET['amp_actioned']) && !empty($_GET['amp_actioned_count'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $actioned = \sanitize_key($_GET['amp_actioned']);
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $count = (int) $_GET['amp_actioned_count'];
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $message = null;
            if ('delete' === $actioned) {
                $message = \sprintf(
                    /* translators: %s is number of errors accepted */
                    \_n('Deleted %s instance of validation errors.', 'Deleted %s instances of validation errors.', \number_format_i18n($count), 'amp'),
                    $count
                );
            }
            if ($message) {
        ...
        if (isset($_GET[self::VALIDATION_ERRORS_CLEARED_QUERY_VAR])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $cleared_count = (int) $_GET[self::VALIDATION_ERRORS_CLEARED_QUERY_VAR];
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            \printf('<div class="notice notice-success is-dismissible"><p>%s</p></div>', \esc_html(\sprintf(
                /* translators: %s is the number of validation errors cleared */
                \_n('Cleared %s validation error for invalid markup that no longer occurs on the site.', 'Cleared %s validation errors for invalid markup that no longer occur on the site.', $cleared_count, 'amp'),
                \number_format_i18n($cleared_count)
            )));
        }
    }
    /**
    ...
    {
        $post = \get_post();
        if (!$post instanceof \WP_Post || \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG !== $post->post_type) {
            return null;
        }
    ...
    public static function filter_tag_row_actions($actions, \WP_Term $tag)
    {
        ...
        // We don't want filter=display given by $tag.
        /*
        ...
        if ('post.php' === $pagenow) {
            ...
            $actions['details'] = \sprintf('<a href="%s">%s</a>', \admin_url(\add_query_arg([self::TAXONOMY_SLUG => $term->name, 'post_type' => \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG], 'edit.php')), \esc_html__('Details', 'amp'));
            if (0 === $term->count) {
                $actions['delete'] = \sprintf('<a href="%s">%s</a>', \wp_nonce_url(\add_query_arg(\array_merge(['action' => 'delete'], \compact('term_id'))), 'delete'), \esc_html__('Delete', 'amp'));
            }
        }
    ...
    public static function add_admin_menu_validation_error_item()
    {
        ...
        global $submenu;
        if (\current_user_can('manage_options')) {
            $taxonomy_caps = (object) \get_taxonomy(self::TAXONOMY_SLUG)->cap;
            // Yes, cap is an object not an array.
            \add_submenu_page(
                \Google\Web_Stories_Dependencies\AMP_Options_Manager::OPTION_NAME,
                $menu_item_label,
                $menu_item_label,
    ...
    public static function filter_manage_custom_columns($content, $column_name, $term_id)
    {
        ...
        switch ($column_name) {
            case 'error_code':
                if ('post.php' === $pagenow) {
                    $content .= \sprintf('<button type="button" aria-label="%s" class="single-url-detail-toggle">', \esc_attr__('Toggle error details', 'amp'));
                    $content .= \wp_kses_post(self::get_error_title_from_code($validation_error));
                } else {
                    $content .= '<p>';
                    $content .= \sprintf('<a class="row-title" href="%s">%s', \admin_url(\add_query_arg([self::TAXONOMY_SLUG => $term->name, 'post_type' => \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG], 'edit.php')), \wp_kses_post(self::get_error_title_from_code($validation_error)));
                }
                if ('post.php' === $pagenow) {
                    $content .= '</button>';
                } else {
                ...
                switch ($validation_error['code']) {
                    case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::JSON_ERROR_EMPTY:
                        $message = \__('Expected JSON, got an empty value', 'amp');
                        break;
                    case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::JSON_ERROR_DEPTH:
                        $message = \__('The maximum stack depth has been exceeded', 'amp');
                        break;
                    case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::JSON_ERROR_STATE_MISMATCH:
                        $message = \__('Invalid or malformed JSON', 'amp');
                        break;
                    case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::JSON_ERROR_CTRL_CHAR:
                        $message = \__('Control character error, possibly incorrectly encoded', 'amp');
                        break;
                    case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::JSON_ERROR_SYNTAX:
                        $message = \__('Syntax error', 'amp');
                        break;
                    case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::JSON_ERROR_UTF8:
            ...
            case 'status':
                ...
                if ('post.php' === $pagenow) {
                    $status_select_name = \sprintf('%s[%s][%s]', \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY, $term->slug, \Google\Web_Stories_Dependencies\AMP_Validation_Manager::VALIDATION_ERROR_TERM_STATUS_QUERY_VAR);
                    \ob_start();
                    ?>
					<div class="amp-validation-error-status-dropdown">
						<label for="<?php 
                    echo \esc_attr($status_select_name);
                    ?>" class="screen-reader-text">
							<?php 
                    \esc_html_e('Markup Status', 'amp');
                    ?>
						</label>
						<select class="amp-validation-error-status" name="<?php 
                    echo \esc_attr($status_select_name);
                    ?>">
							<option value="<?php 
                    echo \esc_attr(self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS);
                    ?>" <?php 
                    \selected($is_removed);
                    ?>>
								<?php 
                    \esc_html_e('Removed', 'amp');
                    ?>
							</option>
							<option value="<?php 
                    echo \esc_attr(self::VALIDATION_ERROR_ACK_REJECTED_STATUS);
                    ?>" <?php 
                    \selected(!$is_removed);
                    ?>>
								<?php 
            ...
            case 'created_date_gmt':
                ...
                $created_date_gmt = \get_term_meta($term_id, 'created_date_gmt', \true);
                if ($created_date_gmt) {
                    try {
                        $created_datetime = new \DateTime($created_date_gmt, new \DateTimeZone('UTC'));
                        $timezone_string = \get_option('timezone_string');
                        if (!$timezone_string && \get_option('gmt_offset')) {
                            $timezone_string = \timezone_name_from_abbr('', \get_option('gmt_offset') * \HOUR_IN_SECONDS, \false);
                        }
                        if ($timezone_string) {
                ...
                if (!$created_datetime) {
                    ...
                    $time_ago = \sprintf(
                        '<abbr title="%s">%s</abbr>',
                        \esc_attr($created_datetime->format(
                ...
                if ($created_datetime) {
                    $time_ago = \sprintf('<time datetime="%s">%s</time>', $created_datetime->format('c'), $time_ago);
                }
                $content .= $time_ago;
                break;
            case 'details':
            ...
            case 'sources_with_invalid_output':
                ...
                $url_post_id = (int) $_GET['post'];
                // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                $validation_errors = \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::get_invalid_url_validation_errors($url_post_id);
                $validation_errors = \array_filter($validation_errors, static function ($error) use($term) {
                ...
                if (empty($error_summary[self::SOURCES_INVALID_OUTPUT])) {
                    ...
                    \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::render_sources_column($error_summary[self::SOURCES_INVALID_OUTPUT], $url_post_id);
                }
                break;
            case 'error_type':
                if (isset($validation_error['type'])) {
                    ...
                    if ('post.php' === $pagenow) {
                        $content .= \sprintf('<p data-error-type="%s">%s</p>', isset($validation_error['type']) ? $validation_error['type'] : '', \esc_html($text));
                    } else {
                        $content .= $text;
                    }
                } else {
            ...
            case 'reviewed':
                if ('post.php' === $pagenow) {
                    $checked = \checked(0 < ((int) $term->term_group & self::ACKNOWLEDGED_VALIDATION_ERROR_BIT_MASK), \true, \false);
                    $input_name = \sprintf('%s[%s][%s]', \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY, $term->slug, self::VALIDATION_ERROR_ACKNOWLEDGE_ACTION);
    ...
    {
        return \array_merge($sortable_columns, ['error_code' => self::VALIDATION_DETAILS_ERROR_CODE_QUERY_VAR, 'error_type' => self::VALIDATION_ERROR_TYPE_QUERY_VAR]);
    }
    /**
     ...
     */
    public static function render_single_url_error_details($validation_error, $term, $wrap_with_details = \true, $with_summary = \true)
    {
        // Get the sources, if they exist.
        if (isset($_GET['post'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $validation_errors = \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::get_invalid_url_validation_errors((int) $_GET['post']);
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		<dl class="detailed">
			<dt><?php 
        \esc_html_e('Information', 'amp');
        ?></dt>
			<dd class="detailed">
				<p>
					<?php 
        if (isset($validation_error['type']) && self::JS_ERROR_TYPE === $validation_error['type']) {
            ?>
						<?php 
            ...
            ?>
					<?php 
        } elseif (isset($validation_error['type']) && self::CSS_ERROR_TYPE === $validation_error['type']) {
            ?>
						<?php 
            echo \wp_kses_post(\sprintf(
        ...
        ?>
				...
				<p>
					<?php 
        echo \wp_kses_post(\__('If all invalid markup is &#8220;removed&#8221; the page will be served as AMP. However, the impact that the removal has on the page must be assessed to determine if the result is acceptable. If any invalid markup is &#8220;kept&#8221; then the page will not be served as AMP.', 'amp'));
        ?>
				</p>
			</dd>

			<dt><?php 
        \esc_html_e('Error code', 'amp');
        ?></dt>
			<dd><code><?php 
        echo \esc_html($validation_error['code']);
        ?></code></dd>

			<?php 
        if (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_TAG === $validation_error['code'] && isset($validation_error['node_attributes'])) {
            ?>
				<dt><?php 
            \esc_html_e('Invalid markup', 'amp');
            ?></dt>
				<dd class="detailed">
					<code>
						<mark>
			...
			<?php 
        } elseif (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_ATTR === $validation_error['code'] && isset($validation_error['element_attributes'])) {
            ?>
				<dt><?php 
            \esc_html_e('Invalid markup', 'amp');
            ?></dt>
        ...
        foreach ($validation_error as $key => $value) {
            ...
            if (\in_array($key, ['code', 'type', 'css_property_value', 'mandatory_anyof_attrs', 'meta_property_value', 'meta_property_required_value', 'mandatory_oneof_attrs'], \true)) {
                continue;
                // Handled above.
            }
            if (\in_array($key, ['spec_name', 'tag_spec', 'spec_names', 'node_type', 'allowed_descendants'], \true)) {
                continue;
            }
            ?>
				<dt><?php 
            echo \esc_html(self::get_source_key_label($key, $validation_error));
            ?></dt>
				<dd class="detailed">
					<?php 
            if (\in_array($key, ['node_name', 'parent_name', 'required_parent_name', 'required_attr_value', 'css_selector', 'class_name'], \true)) {
                ?>
						<code><?php 
                echo \esc_html($value);
                ?></code>
					<?php 
            } elseif ('css_property_name' === $key) {
                ?>
						<?php 
                if (isset($validation_error['css_property_value']) && \is_scalar($validation_error['css_property_value'])) {
                    \printf('<code>%s: %s</code>', \esc_html($value), \esc_html($validation_error['css_property_value']));
                } else {
                    \printf('<code>%s</code>', \esc_html($value));
                }
                ?>
					<?php 
            } elseif ('meta_property_name' === $key) {
                ?>
						<?php 
                \printf('<code>%s</code>', \esc_html($value));
                ...
                if (isset($validation_error['meta_property_required_value'])) {
                    \printf(' (%s: <code>%s</code>)', \esc_html__('required value', 'amp'), \esc_html($validation_error['meta_property_required_value']));
                }
                ?>
					<?php 
            } elseif ('text' === $key) {
                ?>
						<?php 
                self::render_code_details($value);
                ?>
					<?php 
            } elseif ('sources' === $key) {
                ?>
						<?php 
                self::render_sources($value);
                ?>
					<?php 
                ...
                foreach ($value as $attr_name => $attr_value) {
                    ...
                    if (!empty($attr_value)) {
                        echo '<code>';
                        $is_url = \in_array($attr_name, ['href', 'src'], \true);
                        if ($is_url) {
                ...
                ?>
					...
					<?php 
            } elseif ('duplicate_oneof_attrs' === $key) {
                ?>
						<ul>
						<?php 
                foreach ($value as $attr) {
                ...
                ?>
						<?php 
                foreach ($value as $value_key => $attr) {
                    ?>
							<?php 
                    if (\is_int($value_key)) {
                        echo \esc_html($attr);
    ...
    private static function get_file_editor_url($source)
    {
        ...
        if (null !== $editor_url_template && \false !== \strpos($editor_url_template, '{{file}}')) {
            ...
            if ($file_path && \file_exists($file_path)) {
                ...
                if ($file_path) {
                    $edit_url = \str_replace(['{{file}}', '{{line}}'], [\rawurlencode($file_path), \rawurlencode($source['line'])], $editor_url_template);
                }
            }
        }
        ...
        if (!$edit_url) {
            if ('plugin' === $source['type'] && \current_user_can('edit_plugins')) {
                ...
                if ($plugin) {
                    ...
                    $edit_url = \add_query_arg(['plugin' => \rawurlencode($plugin['file']), 'file' => \rawurlencode($file), 'line' => \rawurlencode($source['line'])], \admin_url('plugin-editor.php'));
                }
            } elseif ('theme' === $source['type'] && \current_user_can('edit_themes')) {
                $edit_url = \add_query_arg(['file' => \rawurlencode($source['file']), 'theme' => \rawurlencode($source['name']), 'line' => \rawurlencode($source['line'])], \admin_url('theme-editor.php'));
            }
        }
        return $edit_url;
    }
    ...
    public static function render_sources($sources)
    {
        ...
        echo \esc_html(\sprintf(
            ...
            \_n('Source stack (%s)', 'Source stack (%s)', $source_count, 'amp'),
            \number_format_i18n($source_count)
        ));
        ?>
			</summary>
        ...
        foreach ($sources as $i => $source) {
            ...
            if (isset($source['file'], $source['line'])) {
                ...
                $source_table_rows['location'] = ['link_text' => $source['file'] . ':' . $source['line'], 'link_url' => self::get_file_editor_url($source)];
            }
            ...
            ?>
					<tbody>
						<?php 
            foreach (\array_keys($source_table_rows) as $j => $key) {
                ?>
							<?php 
                ...
                if ('sources' === $key && \is_array($value)) {
                    ...
                    ?>
									<?php 
                } elseif ('name' === $key && isset($source['type'])) {
                    ?>
										<?php 
                    self::render_source_name($value, $source['type']);
                    ?>
									<?php 
                } elseif ('extra_key' === $key) {
                    ?>
										<?php 
                    if ('style' === $dependency_type) {
                        ?>
											<code>wp_add_inline_style( <?php 
                        echo \esc_html(\wp_json_encode($source['handle']));
                        ?>, &hellip; )</code>
										<?php 
                    } elseif ('data' === $value) {
                        ?>
											<code>wp_localize_script( <?php 
                        echo \esc_html(\wp_json_encode($source['handle']));
                        ?>, &hellip; )</code>
										<?php 
                    } elseif ('before' === $value) {
                        ?>
											<code>wp_add_inline_script( <?php 
                        echo \esc_html(\wp_json_encode($source['handle']));
                        ?>, &hellip;, 'before' )</code>
										<?php 
                    } elseif ('after' === $value) {
                    ...
                    if (!empty($value['link_url'])) {
                        \printf(
                            ...
                            \in_array(\wp_parse_url($value['link_url'], \PHP_URL_SCHEME), ['http', 'https'], \true) ? 'target="_blank"' : ''
                        );
                    }
                    ?>
										<?php 
                    echo \esc_html($value['link_text']);
                    ?>
										<?php 
                    if (!empty($value['link_url'])) {
                        ?>
                    ...
                    ?>
										<code><?php 
                    echo \esc_html('{closure}' === $value ? $value : $value . '()');
                    ?></code>
									<?php 
                } elseif ('shortcode' === $key || 'handle' === $key || 'dependency_handle' === $key) {
                    ?>
										<code><?php 
                    echo \esc_html($value);
                    ?></code>
									<?php 
                } elseif ('block_name' === $key) {
                    ?>
										<?php 
                    ...
                    ?>
										<pre><?php 
                    echo \esc_html(\wp_json_encode($value, 128 | 64));
                    ?></pre>
									<?php 
                }
                ?>
    ...
    public static function get_translated_type_name($validation_error)
    {
        ...
        $translated_names = [self::HTML_ELEMENT_ERROR_TYPE => \__('HTML Element', 'amp'), self::HTML_ATTRIBUTE_ERROR_TYPE => \__('HTML Attribute', 'amp'), self::JS_ERROR_TYPE => \__('JavaScript', 'amp'), self::CSS_ERROR_TYPE => \__('CSS', 'amp')];
    ...
    public static function handle_inline_edit_request()
    {
        ...
        if (!isset($_GET['action'], $_GET['_wpnonce'], $_GET['term_id'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        // Check if we are on either the taxonomy page or a single error page (which has the post_type argument).
        if (self::TAXONOMY_SLUG !== \get_current_screen()->taxonomy && !isset($_GET['post_type'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        // If we have a post_type check that it is the correct one.
        if (isset($_GET['post_type']) && \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG !== $_GET['post_type']) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
    ...
    {
        if (!isset($_REQUEST['action']) || \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG !== \get_post_type($post_id)) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        $action = \sanitize_key($_REQUEST['action']);
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $term_ids = isset($_POST['delete_tags']) ? \array_filter(\array_map('intval', (array) $_POST['delete_tags'])) : [];
        // phpcs:ignore WordPress.Security.NonceVerification.Missing
        $single_term_id = isset($_GET['term_id']) ? (int) $_GET['term_id'] : null;
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $redirect_query_args = ['action' => 'edit', 'amp_actioned' => $action];
        if ($term_ids) {
    ...
    {
        return isset($validation_error['node_name']) && 'script' === $validation_error['node_name'] && (isset($validation_error['node_attributes']['src']) || empty($validation_error['node_attributes']['type']) || \false !== \strpos($validation_error['node_attributes']['type'], 'javascript'));
    }
    /**
    ...
    {
        switch ($validation_error['code']) {
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_TAG:
                ...
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_ATTR:
                return \sprintf('%s: <code>%s</code>', \esc_html__('Invalid attribute', 'amp'), \esc_html($validation_error['node_name']));
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_PROCESSING_INSTRUCTION:
                return \sprintf('%s: <code>&lt;%s%s&hellip;%s&gt;</code>', \esc_html__('Invalid processing instruction', 'amp'), '?', \esc_html($validation_error['node_name']), '?');
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::STYLESHEET_TOO_LONG:
                return \esc_html__('Excessive CSS', 'amp');
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::CSS_SYNTAX_INVALID_AT_RULE:
                return \sprintf('%s: <code>@%s</code>', \esc_html__('Illegal CSS at-rule', 'amp'), \esc_html($validation_error['at_rule']));
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DUPLICATE_UNIQUE_TAG:
                return \sprintf('%s: <code>&lt;%s&gt;</code>', \esc_html__('Duplicate element', 'amp'), \esc_html($validation_error['node_name']));
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::CSS_SYNTAX_INVALID_DECLARATION:
                return \esc_html__('Unrecognized CSS', 'amp');
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::CSS_SYNTAX_PARSE_ERROR:
                return \esc_html__('CSS parse error', 'amp');
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::STYLESHEET_FETCH_ERROR:
                return \esc_html__('Stylesheet fetch error', 'amp');
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::CSS_SYNTAX_INVALID_PROPERTY:
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::CSS_SYNTAX_INVALID_PROPERTY_NOLIST:
                ...
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::CSS_DISALLOWED_SELECTOR:
                return \esc_html__('Illegal CSS selector', 'amp');
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::DISALLOWED_ATTR_CLASS_NAME:
                ...
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::CDATA_TOO_LONG:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MANDATORY_CDATA_MISSING_OR_INCORRECT:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_CDATA_HTML_COMMENTS:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_CDATA_CSS_I_AMPHTML_NAME:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_CDATA_CONTENTS:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::CDATA_VIOLATES_DENYLIST:
            ...
            case \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::CSS_SYNTAX_INVALID_IMPORTANT:
                $title = \esc_html__('Illegal CSS !important property', 'amp');
                if (isset($validation_error['css_property_name'])) {
                    $title .= \sprintf(': <code>%s</code>', \esc_html($validation_error['css_property_name']));
                }
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_PROPERTY_IN_ATTR_VALUE:
                ...
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MISSING_MANDATORY_PROPERTY:
                $title = \esc_html__('Missing required property', 'amp');
                if (isset($validation_error['meta_property_name'])) {
                    $title .= \sprintf(': <code>%s</code>', \esc_html($validation_error['meta_property_name']));
                }
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MISSING_REQUIRED_PROPERTY_VALUE:
                $title = \sprintf(
                ...
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::ATTR_REQUIRED_BUT_MISSING:
                ...
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DUPLICATE_ONEOF_ATTRS:
                $title = \esc_html__('Mutually exclusive attributes encountered', 'amp');
                if (!empty($validation_error['duplicate_oneof_attrs'])) {
                    $title .= ': ';
                    $title .= \implode(', ', \array_map(static function ($attribute_name) {
                        return \sprintf('<code>%s</code>', $attribute_name);
                    }, $validation_error['duplicate_oneof_attrs']));
                }
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MANDATORY_ONEOF_ATTR_MISSING:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MANDATORY_ANYOF_ATTR_MISSING:
                $attributes_key = null;
                if (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MANDATORY_ONEOF_ATTR_MISSING === $validation_error['code']) {
                ...
                return $title;
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_CHILD_TAG:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_FIRST_CHILD_TAG:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INCORRECT_NUM_CHILD_TAGS:
                return \sprintf(\esc_html(
                    /* translators: %1$s is the node name, %2$s is required child count */
                    \_n('Tag %1$s must have %2$s child tag', 'Tag %1$s must have %2$s child tags', (int) $validation_error['required_child_count'], 'amp')
                ), '<code>' . \esc_html($validation_error['node_name']) . '</code>', \esc_html(\number_format_i18n((int) $validation_error['required_child_count'])));
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INCORRECT_MIN_NUM_CHILD_TAGS:
                return \sprintf(\esc_html(
                    /* translators: %1$s is the node name, %2$s is required child count */
                    \_n('Tag %1$s must have a minimum of %2$s child tag', 'Tag %1$s must have a minimum of %2$s child tags', (int) $validation_error['required_min_child_count'], 'amp')
                ), '<code>' . \esc_html($validation_error['node_name']) . '</code>', \esc_html(\number_format_i18n((int) $validation_error['required_min_child_count'])));
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::WRONG_PARENT_TAG:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_TAG_ANCESTOR:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_DESCENDANT_TAG:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MANDATORY_TAG_ANCESTOR:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_ATTR_VALUE:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_ATTR_VALUE_CASEI:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_ATTR_VALUE_REGEX:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_ATTR_VALUE_REGEX_CASEI:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_DISALLOWED_VALUE_REGEX:
                return \sprintf(
                    /* translators: %s is the attribute name */
                    \esc_html__('The attribute %s is set to an invalid value', 'amp'),
                    '<code>' . \esc_html($validation_error['node_name']) . '</code>'
                );
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_URL_PROTOCOL:
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_URL:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_RELATIVE_URL:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MISSING_URL:
                return \sprintf(
                    /* translators: %1$s is attribute name */
                    \esc_html__('Missing URL for attribute %s', 'amp'),
                    '<code>' . \esc_html($validation_error['node_name']) . '</code>'
                );
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DUPLICATE_DIMENSIONS:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_WIDTH:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_HEIGHT:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_AUTO_HEIGHT:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_NO_HEIGHT:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_FIXED_HEIGHT:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_AUTO_WIDTH:
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_HEIGHTS:
                if (isset($validation_error['node_attributes'][$validation_error['attribute']])) {
                    return \sprintf(
                        ...
                        empty($validation_error['node_attributes'][$validation_error['attribute']]) ? \esc_html__('(empty)', 'amp') : '<code>' . \esc_html($validation_error['node_attributes'][$validation_error['attribute']]) . '</code>',
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INVALID_LAYOUT_UNIT_DIMENSIONS:
                return \esc_html__('Inconsistent units for width and height', 'amp');
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MISSING_LAYOUT_ATTRIBUTES:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::IMPLIED_LAYOUT_INVALID:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::SPECIFIED_LAYOUT_INVALID:
                return \sprintf(
                    /* translators: %1$s is the layout, %2$s is the tag */
            ...
            case \Google\Web_Stories_Dependencies\AMP_Form_Sanitizer::POST_FORM_HAS_ACTION_XHR_WHEN_NATIVE_USED:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Script_Sanitizer::CUSTOM_EXTERNAL_SCRIPT:
                return \sprintf(
            ...
            case \Google\Web_Stories_Dependencies\AMP_Script_Sanitizer::CUSTOM_INLINE_SCRIPT:
                return \esc_html__('Custom inline script encountered', 'amp');
            case \Google\Web_Stories_Dependencies\AMP_Script_Sanitizer::CUSTOM_EVENT_HANDLER_ATTR:
                return \sprintf(
    ...
    public static function get_source_key_label($key, $validation_error)
    {
        switch ($key) {
            ...
            case 'node_name':
                if (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_ATTR === $validation_error['code']) {
                    return \__('Attribute name', 'amp');
                } elseif (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_TAG === $validation_error['code']) {
            ...
            case 'meta_property_name':
                if (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_PROPERTY_IN_ATTR_VALUE === $validation_error['code'] || \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MISSING_REQUIRED_PROPERTY_VALUE === $validation_error['code']) {
                    return \__('Invalid property', 'amp');
                } elseif (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MISSING_MANDATORY_PROPERTY === $validation_error['code']) {
                    return \__('Missing property', 'amp');
                }
                return \__('Property name', 'amp');
            case 'property_value':
                if (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_PROPERTY_IN_ATTR_VALUE === $validation_error['code']) {
                    return \__('Invalid property value', 'amp');
                } elseif (\Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::MISSING_REQUIRED_PROPERTY_VALUE === $validation_error['code']) {
