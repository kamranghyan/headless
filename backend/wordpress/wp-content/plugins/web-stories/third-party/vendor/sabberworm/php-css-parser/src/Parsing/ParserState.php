...
use Google\Web_Stories_Dependencies\Sabberworm\CSS\Comment\Comment;
use Google\Web_Stories_Dependencies\Sabberworm\CSS\Settings;
class ParserState
{
    /**
     * @var null
        ...
        if ($this->peek() === '\\') {
            ...
            if ($this->strlen($sUnicode) < 6) {
                ...
                if (\preg_match('/\\s/isSu', $this->peek())) {
                    if ($this->comes('Google\\Web_Stories_Dependencies\\r\\n')) {
                        $this->consume(2);
                    } else {
                        $this->consume(1);
                    }
